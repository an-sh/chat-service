...
export default function() {
    ...
    , function() {
      ...
      return socket1.on('loginConfirmed', () =>
        socket1.emit('roomJoin', roomName1, () =>
          socket1.emit('roomGetWhitelistMode', roomName1, function(error, data) {
            expect(error).not.ok;
            expect(data).true;
            return done();
          }
  ...
  it('should allow duplicate adding to lists', function(done) {
    ...
    socket1 = clientConnect(user1);
    return socket1.on('loginConfirmed', u =>
  ...
  it('should check room permissions', function(done) {
    ...
    return chatService.addRoom(roomName1, { blacklist : [user1] }, function() {
      socket1 = clientConnect(user1);
      return socket1.on('loginConfirmed', () =>
        socket1.emit('roomJoin', roomName1, function(error, data) {
          expect(error).ok;
          expect(data).null;
          return done();
        }
    ...
    , function() {
      ...
      return socket1.on('loginConfirmed', () =>
        socket1.emit('roomJoin', roomName1, function(error, data) {
          ...
          socket2 = clientConnect(user2);
          return socket2.on('loginConfirmed', () =>
            socket2.emit('roomJoin', roomName1, function(error, data) {
              expect(error).not.ok;
              expect(data).equal(1);
              return done();
            }
  ...
  it('should remove users on permissions changes', function(done) {
    chatService = startService();
    return chatService.addRoom(roomName1, { adminlist: [user1, user3] }, () =>
      parallel([
        function(cb) {
          socket1 = clientConnect(user1);
        ...
        return parallel([
          ...
          ,
          cb =>
            socket2.on('roomAccessRemoved', function(r) {
              expect(r).equal(roomName1);
              return cb();
            }
            )
          ,
          cb =>
            socket1.on('roomUserLeft', function(r, u) {
              expect(r).equal(roomName1);
    ...
    return chatService.addRoom(roomName1, { adminlist : [user1] }, function() {
      ...
      return socket1.on('loginConfirmed', () =>
        socket1.emit('roomJoin', roomName1, function() {
          socket2 = clientConnect(user2);
          return socket2.on('loginConfirmed', () =>
            socket2.emit('roomJoin', roomName1, function() {
              socket1.emit('roomSetWhitelistMode', roomName1, true);
      ...
      return socket1.on('loginConfirmed', () =>
        socket1.emit('roomJoin', roomName1, function() {
          ...
          return socket2.on('loginConfirmed', () =>
            socket2.emit('roomJoin', roomName1, function() {
              socket3 = clientConnect(user3);
              return socket3.on('loginConfirmed', () =>
                socket3.emit('roomJoin', roomName1, function() {
