...
import { ChatService, cleanup, clientConnect, nextTick, parallel, startService } from './testutils.coffee';
...
export default function() {
  ...
  it('should support multiple sockets per user', function(done) {
    ...
    return socket1.on('loginConfirmed', function() {
      ...
      return parallel([
        cb =>
          socket1.on('socketConnectEcho', function(id, nconnected) {
            sid2e = id;
        ...
        ,
        cb =>
          socket2.on('loginConfirmed', function(u, data) {
            sid2 = data.id;
            return cb();
          }
          )
