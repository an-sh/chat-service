...
export default function() {
  ...
  it('should recover from rollback room join errors', function(done) {
    ...
    return instance1.addRoom(roomName1, null, function() {
      ...
      return socket1.on('loginConfirmed', (userName, { id }) =>
        ...
        .then(function(user) {
          ...
          return parallel([
            ...
            ,
            cb =>
              instance1.on('storeConsistencyFailure', function(error, data) {
                nextTick(function() {
                  expect(error).ok;
      ...
      return socket1.on('loginConfirmed',(userName, { id })  =>
        socket1.emit('roomJoin', roomName1, () =>
          ...
          .then(function(room) {
            ...
            return parallel([
              cb =>
                socket1.emit('roomLeave', roomName1, function(error, data) {
                  expect(error).not.ok;
                  return cb();
                }
                )
      ...
      return socket1.on('loginConfirmed', (userName, { id }) =>
        socket1.emit('roomJoin', roomName1, () =>
          ...
          .then(function(user) {
            ...
            socket1.disconnect();
            return instance1.on('storeConsistencyFailure', (error, data) =>
              nextTick(function() {
                user.userState.__proto__.removeSocket = orig;
                expect(error).ok;
                expect(data).an('Object');
  ...
  it('should recover from room access check errors', function(done) {
    instance1 = startService();
    return instance1.addRoom(roomName1, null, () =>
      instance1.state.getRoom(roomName1).then(function(room) {
        socket1 = clientConnect(user1);
        return socket1.on('loginConfirmed', (userName, { id }) =>
  ...
  return it('should emit consistencyFailure on leave channel errors', function(done) {
    ...
    return instance1.addRoom(roomName1, null, function() {
      socket1 = clientConnect(user1);
      return socket1.on('loginConfirmed', (userName, { id }) =>
        socket1.emit('roomJoin', roomName1, function(error) {
          expect(error).not.ok;
          return parallel([
            cb =>
              socket1.emit('roomLeave', roomName1, function(error) {
                expect(error).not.ok;
                return cb();
              }
              )
            ,
            cb =>
              instance1.on('transportConsistencyFailure', function(error, data) {
                nextTick(function() {
                  expect(error).ok;
                  expect(data).an('Object');
