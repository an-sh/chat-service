...
export default function() {
  ...
  it('should reject delete rooms for non-owners', function(done) {
    ...
    return socket1.on('loginConfirmed', function(u) {
      ...
      return socket2.on('loginConfirmed', u =>
        socket2.emit('roomDelete', roomName1, function(error, data) {
          expect(error).ok;
          expect(data).null;
          return done();
        }
  ...
  it('should check for invalid room names', function(done) {
    ...
    socket1 = clientConnect(user1);
    return socket1.on('loginConfirmed', u =>
      socket1.emit('roomCreate', 'room}1', false, function(error, data) {
        expect(error).ok;
        expect(data).null;
        return done();
      }
  ...
  it('should reject room management when the option is disabled', function(done) {
    ...
    return chatService.addRoom(roomName2, null, function() {
      socket1 = clientConnect(user1);
      return socket1.on('loginConfirmed', u =>
        socket1.emit('roomCreate', roomName1, false, function(error, data) {
          expect(error).ok;
          expect(data).null;
    ...
    return chatService.addRoom(roomName1, { owner : user1 }, () =>
      parallel([
        ...
        return parallel([
          ...
          cb =>
            socket1.on('roomAccessRemoved', function(r) {
              expect(r).equal(roomName1);
              return cb();
            }
            )
