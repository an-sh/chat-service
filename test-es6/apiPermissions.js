...
export default function() {
  ...
  it('should get a user mode', function (done) {
    ...
    return chatService.addUser(user1, { whitelistOnly: true }, () => chatService.execUserCommand(user1, 'directGetWhitelistMode'
      , function (error, data) {
        expect(error).not.ok
        expect(data).true
        return done()
  ...
  it('should change user lists', function (done) {
    chatService = startService()
    return chatService.addUser(user1, null, () => chatService.execUserCommand(user1
  ...
  it('should get a room mode', function (done) {
    chatService = startService()
    return chatService.addRoom(roomName1, { whitelistOnly: true }, () => chatService.execUserCommand(true
  ...
  it('should change room lists', function (done) {
    chatService = startService()
    return chatService.addRoom(roomName1, null, () => chatService.execUserCommand(true
  ...
  it('should send system messages to all user sockets', function (done) {
    ...
    return socket1.on('loginConfirmed', function () {
      ...
      return socket2.on('loginConfirmed', () => parallel([
        cb => chatService.execUserCommand(user1, 'systemMessage', data, cb),
        cb => socket1.on('systemMessage', function (d) {
          expect(d).equal(data)
          return cb()
        }
        )
        ,
        cb => socket2.on('systemMessage', function (d) {
          expect(d).equal(data)
          return cb()
        }
        )
  ...
  it('should execute commands without hooks', function (done) {
    ...
    return chatService.addRoom(roomName1, { owner: user1 }, () => chatService.addUser(user2, null, function () {
      socket1 = clientConnect(user1)
      return socket1.on('loginConfirmed', () => socket1.emit('roomJoin', roomName1, () => chatService.execUserCommand({ userName: user1,               bypassHooks: true }
  ...
  it('should bypass user messaging permissions', function (done) {
    ...
    return chatService.addUser(user1, null, function () {
      ...
      return socket2.on('loginConfirmed', function () {
        chatService.execUserCommand({ userName: user1,           bypassPermissions: true }
    ...
    return chatService.addRoom(roomName1
      ...
      , () => chatService.addUser(user2, null, function () {
        ...
        return socket1.on('loginConfirmed', () => socket1.emit('roomJoin', roomName1, function () {
          chatService.execUserCommand({ userName: user2,               bypassPermissions: true }
  ...
  it('should not allow using non-existing users', function (done) {
    ...
    let message = { textMessage: txt }
    chatService = startService()
    return chatService.addRoom(roomName1, null, () => chatService.execUserCommand(user1, 'roomMessage', roomName1, message
      , function (error, data) {
        expect(error).ok
        expect(data).not.ok
        return done()
  ...
  it('should check for direct messaging permissions', function (done) {
    chatService = startService()
    socket1 = clientConnect(user1)
    return socket1.on('loginConfirmed', () => socket1.emit('directAddToList', 'blacklist', [user3], function (error) {
      expect(error).not.ok
      return parallel([
        function (cb) {
  ...
  return it('should check for room messaging permissions', function (done) {
    chatService = startService()
    return chatService.addRoom(roomName1, {blacklist: [user3]}, () => parallel([
      function (cb) {
