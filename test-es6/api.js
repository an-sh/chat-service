...
export default function() {
  ...
  it('should support adding users', function(done) {
    ...
    return chatService.addUser(user1, { whitelistOnly : true }, function() {
      ...
      return socket1.on('loginConfirmed', () =>
        socket1.emit('directGetWhitelistMode', function(error, data) {
          expect(error).not.ok;
          expect(data).true;
          return done();
        }
  ...
  it('should support deleting users', function(done) {
    ...
    return socket1.on('loginConfirmed', function() {
      chatService.disconnectUserSockets(user1);
      return socket1.on('disconnect', () =>
        chatService.hasUser(user1, function(error, data) {
          expect(error).not.ok;
          expect(data).true;
  ...
  it('should not delete connected users', function(done) {
    ...
    socket1 = clientConnect(user1);
    return socket1.on('loginConfirmed', () =>
      chatService.deleteUser(user1, function(error) {
        expect(error).ok;
        return done();
      }
      )
  ...
  it('should support deleting rooms', function(done) {
    ...
    return chatService.addRoom(roomName1, { owner : user1 }, () =>
      chatService.hasRoom(roomName1, function(error, data) {
        expect(error).not.ok;
        expect(data).true;
  ...
  it('should check existing users before adding new ones', function(done) {
    ...
    return chatService.addUser(user1, null, () =>
      chatService.addUser(user1, null, function(error, data) {
        expect(error).ok;
        expect(data).not.ok;
        return done();
      }
  ...
  it('should check commands names.', function(done) {
    chatService = startService();
    return chatService.addUser(user1, null, () =>
      chatService.execUserCommand(user1, 'nocmd', function(error) {
        expect(error).ok;
        return done();
      }
      )
  ...
  it('should check for socket ids if required.', function(done) {
    chatService = startService();
    return chatService.addUser(user1, null, () =>
      chatService.execUserCommand(user1, 'roomJoin', function(error) {
        expect(error).ok;
        return done();
      }
      )
    ...
    return chatService.addRoom(roomName1, { owner : user1 }, function() {
      ...
      return socket1.on('loginConfirmed', () =>
        socket1.emit('roomJoin', roomName1, () =>
          chatService.changeRoomOwner(roomName1, user2, function(error, data) {
            expect(error).not.ok;
            expect(data).not.ok;
  ...
  it('should support changing a room history limit', function(done) {
    ...
    return chatService.addRoom(roomName1, null, () =>
      chatService.changeRoomHistoryMaxSize(roomName1, sz, function(error, data) {
        expect(error).not.ok;
        expect(data).not.ok;
        socket1 = clientConnect(user1);
        return socket1.on('loginConfirmed', () =>
  ...
  it('should support room list checking', function(done) {
    chatService = startService();
    return chatService.addRoom(roomName1, { adminlist : [user1] }, () =>
  ...
  return it('should support user list checking', function(done) {
    chatService = startService();
    return chatService.addUser(user1, { blacklist : [user2] }, () =>
