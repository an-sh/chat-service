...
let UserAssociations = {
  ...
  socketJoinEcho(id, roomName, njoined) {
    return this.transport.sendToChannel(id, this.echoChannel, 'roomJoinedEcho'
    , roomName, id, njoined);
  },

  // @private
  socketLeftEcho(id, roomName, njoined) {
    return this.transport.sendToChannel(id, this.echoChannel, 'roomLeftEcho'
    , roomName, id, njoined);
  },

  // @private
  socketConnectEcho(id, nconnected) {
    return this.transport.sendToChannel(id, this.echoChannel, 'socketConnectEcho', id
    , nconnected);
  },

  // @private
  socketDisconnectEcho(id, nconnected) {
    return this.transport.sendToChannel(id, this.echoChannel, 'socketDisconnectEcho', id
    , nconnected);
  },

  // @private
  leaveChannel(id, channel) {
    return this.transport.leaveChannel(id, channel)
    .catch(e => {
      return this.consistencyFailure(e, { roomName : channel
  ...
  leaveChannelMessage(id, channel) {
    ...
    .timeout(this.server.busAckTimeout)
    .catchReturn();
  },
  ...
  rollbackRoomJoin(error, roomName, id) {
    ...
    .then(njoined => {
      if (!njoined) { return this.leaveRoom(roomName); }
    }
    )
    .thenThrow(error);
  },
  ...
  joinSocketToRoom(id, roomName) {
    return Promise.using(this.userState.lockToRoom(roomName, this.lockTTL), () => {
      return this.state.getRoom(roomName)
      .then(room => {
        return room.join(this.userName)
        .then(() => {
          return this.userState.addSocketToRoom(id, roomName)
          .then(njoined => {
            return this.transport.joinChannel(id, roomName)
            .then(() => {
              if (njoined === 1) {
  ...
  removeUserSocketsFromRoom(roomName) {
    return this.userState.removeAllSocketsFromRoom(roomName)
    .catch(e => {
      return this.consistencyFailure(e, { roomName, opType : 'roomUserlist' });
    }
    );
